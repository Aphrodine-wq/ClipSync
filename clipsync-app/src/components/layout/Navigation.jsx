      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="flex items-center justify-between h-16">
          ...
          <button
            ...
            className="md:hidden p-2 rounded-xl transition-colors glass"
          >
            {showMobileMenu ? (
              <X className="w-6 h-6" style={{ color: 'var(--color-text-secondary)' }} strokeWidth={2} />
            ) : (
              <Menu className="w-6 h-6" style={{ color: 'var(--color-text-secondary)' }} strokeWidth={2} />
            )}
          </button>
              ...
              {tabs.map(tab => {
                ...
                return (
                  <button
                    ...
                    `}
                  >
                    <Icon className="w-4 h-4" style={{ color: isActive ? 'var(--color-text-primary)' : 'var(--color-text-secondary)' }} strokeWidth={2} />
                    <span style={{ color: isActive ? 'var(--color-text-primary)' : 'var(--color-text-secondary)' }}>{tab.label}</span>
                    {tab.badge && (
            ...
            `}>
              ...
              <input
                ...
                className={`
                  ...
                  transition-all duration-200
                  ${searchFocused
                    ? 'glass-strong ring-4 ring-violet-500/10'
                    : ''
                  }
                `}
              />
          ...
          <div className="flex items-center gap-3">
            ...
            {isAuthenticated ? (
              <>
                ...
                <div className="relative">
                  <button
                    ...
                    className="relative p-2 rounded-xl transition-colors glass"
                  >
                    <Bell className="w-5 h-5" style={{ color: 'var(--color-text-secondary)' }} strokeWidth={2} />
                  ...
                  {showNotifications && (
                    <div
                      className="absolute right-0 mt-2 w-80 rounded-2xl shadow-xl py-2 animate-fadeIn glass-strong"
                ...
                <button
                  ...
                  className="p-2 rounded-xl transition-colors glass"
                >
                  <Settings className="w-5 h-5" style={{ color: 'var(--color-text-secondary)' }} strokeWidth={2} />
                </button>
                ...
                <div className="relative">
                  <button
                    ...
                    className="flex items-center gap-2 p-1.5 rounded-xl transition-colors glass"
                  >
                    {user?.picture ? (
                      <img
                        src={user.picture}
                  ...
                  {showUserMenu && (
                    <div
                      className="absolute right-0 mt-2 w-64 rounded-2xl shadow-xl py-2 animate-fadeIn glass-strong"
                      onClick={(e) => e.stopPropagation()}
                    >
                      {/* User Info */}
                      ...
                      <div className="py-2">
                        ...
                        >
                          ...
                          <div className={`w-8 h-5 rounded-full transition-colors ${theme === 'dark' ? 'bg-violet-500' : 'bg-zinc-200'}`}>
                            <div className={`w-4 h-4 bg-white rounded-full shadow transition-transform mt-0.5 ${theme === 'dark' ? 'translate-x-3.5 ml-0' : 'ml-0.5'}`} />
                          </div>
                        </button>
                        <button className="w-full flex items-center gap-3 px-4 py-2 text-sm text-zinc-700 hover:bg-zinc-50 transition-colors">
              ...
              <>
                <button
                  onClick={onPricingClick}
                  className="px-3 py-2 rounded-lg text-sm font-medium transition-colors glass"
                >
                  Pricing
                </button>
                <Button
