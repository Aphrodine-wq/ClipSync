...
import { ClipSyncPanel } from './webview/clipSyncPanel';
...
export function activate(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('clipsync.openPanel', () => {
            ClipSyncPanel.createOrShow(
    ...
    // Save as Template
    context.subscriptions.push(
        vscode.commands.registerCommand('clipsync.saveAsTemplate', async () => {
            const editor = vscode.window.activeTextEditor;
            if (!editor) {
                vscode.window.showErrorMessage('No active editor');
                return;
            ...
            // Save as template clip (with template flag)
            await clipboardManager.saveClip({
                content: selection,
                type: 'code',
                template: true
            });

            vscode.window.showInformationMessage('Template saved! Use {{placeholders}} for dynamic values.');
        })
    );

    // Split Clip
    context.subscriptions.push(
        vscode.commands.registerCommand('clipsync.splitClip', async () => {
            const editor = vscode.window.activeTextEditor;
            if (!editor) return;
            ...
            const parts = selection.split(actualDelimiter).filter(part => part.trim().length > 0);
            
            if (parts.length <= 1) {
                vscode.window.showWarningMessage('Clip cannot be split - delimiter not found or only one part');
                return;
            }

            // Create clips from split parts
            for (const part of parts) {
                await clipboardManager.saveClip({
                    content: part.trim(),
                    type: editor.document.languageId || 'text'
                });
            }

            vscode.window.showInformationMessage(`Split into ${parts.length} clips`);
        })
    );

    // Paste as Plain Text
    context.subscriptions.push(
        vscode.commands.registerCommand('clipsync.pasteAsPlainText', async () => {
            const editor = vscode.window.activeTextEditor;
            if (!editor) return;
            ...
            try {
                ...
                if (text) {
                    ...
                    vscode.window.showInformationMessage('Pasted as plain text');
                }
            } catch (error: any) {
                vscode.window.showErrorMessage(`Failed to paste: ${error.message}`);
            }
        })
    );
